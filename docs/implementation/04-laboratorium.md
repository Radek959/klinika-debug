...
| `SAMPLE_REJECTED` | `IMPLEMENTED` | Wartość `SAMPLE_REJECTED` w `LAB_SIMULATOR_SCENARIO`, deterministyczny wybór jednej odrzucanej próbki i syntetycznych przyczyn w `packages/domain/src/orders/lab-sample-rejection.ts` (`selectSampleToReject`, `planSampleRejection`, `SAMPLE_REJECTION_REASONS`), jedno zadanie `lab_jobs` z terminalnym callbackiem `REJECTED` (`apps/api/src/lab-simulator/lab-simulator.service.ts`), rozszerzony kontrakt webhooka o `rejectedSamples` (`packages/api-contracts/src/lab-results.ts`, `apps/api/src/lab-callbacks/dto/lab-results-webhook.dto.ts`), transakcyjna obsługa w `apps/api/src/lab-callbacks/lab-callbacks.service.ts`, nowe zdarzenie historii `LAB_SAMPLE_REJECTED`, statusy badań `OrderTestStatus.REJECTED` i migracja `20260907190000_lab_sample_rejected`, prezentacja po polsku w `apps/web/src/orders/OrderDetailsPage.tsx` i `OrderHistorySection.tsx`. Testy: `packages/domain/src/orders/lab-sample-rejection.spec.ts`, `packages/domain/src/orders/order-history.spec.ts`, `packages/domain/src/orders/lab-results.spec.ts`, `apps/api/src/lab-simulator/*.spec.ts`, `apps/api/src/config/env.validation.spec.ts`, `apps/api/test/lab-results.e2e-spec.ts`, `apps/web/src/orders/OrdersUi.test.tsx`, `apps/web/src/orders/OrderHistorySection.test.tsx`, `scripts/test-lab-sample-rejected-migration.cjs`. |
