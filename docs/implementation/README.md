...
| Workshop MVP — controlled bugs | `IMPLEMENTED` | Pięć deterministycznych defektów aktywowanych wyłącznie przez `controlledBug` w panelu `/admin` (`apps/api/src/workshop-config/controlled-bug.ts`), co najwyżej jeden naraz: `PATIENT_GUARDIAN` (wyłącza wyłącznie regułę `GUARDIAN_REQUIRED` w `packages/domain/src/patients/patient-write.ts`), `ORDER_FLOW` (błędne przejście do `SAMPLE_COLLECTED` po pierwszej z 2+ wymaganych próbek, `packages/domain/src/orders/sample-collection.ts`), `API_DIAGNOSTICS` (kontrolowany HTTP 500 na `POST /api/v1/orders/{orderId}/send` dla zlecenia ze zleceniem TSH, przed wywołaniem laboratorium, bez efektów ubocznych integracji), `PATIENT_EDIT_NOT_SAVED` (edycja istniejącego pacjenta zgłasza sukces, ale zapisywana wartość `phone` jest cofana do poprzedniej tuż przed zapisem — `apps/api/src/patients/patients.service.ts`, `update`; pozostałe pola tego samego PATCH zapisują się normalnie), `ORDER_PRIORITY_MAPPING` (defekt czysto frontendowy: backend zawsze zapisuje dokładnie to, co dostał w `POST /api/v1/orders`; katalog badań, pobierany tuż przed wysłaniem formularza nowego zlecenia, niesie tylko neutralny sygnał `orderPriorityRoutingActive` — `apps/api/src/tests-catalog/tests-catalog.service.ts` — a samo błędne mapowanie `URGENT` → `ROUTINE` wykonuje `apps/web/src/orders/NewOrderPage.tsx` razem z `buildCreateOrderPayload` w `apps/web/src/orders/orderFormState.ts`; sygnał czytany na świeżo przy każdym zapisie, więc defekt włączony/wyłączony z `/admin` działa na kolejnym „Utwórz zlecenie” bez odświeżenia strony). Każde miejsce oznaczone komentarzem `WORKSHOP CONTROLLED DEFECT`. Testy: `apps/api/src/patients/patient-write-domain.spec.ts`, `packages/domain/src/orders/sample-collection.spec.ts`, `apps/api/test/workshop-controlled-bugs.e2e-spec.ts`, `apps/api/test/tests-catalog.e2e-spec.ts`, `apps/web/src/orders/orderFormState.test.ts`, `apps/web/src/orders/OrdersUi.test.tsx`. |
