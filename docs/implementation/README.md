...
Scenariusz `RATE_LIMIT` symulatora laboratorium jest zaimplementowany w PR `feat/lab-rate-limit` razem z fundamentem trwałych ponowień wysyłki. Pełne reguły retry (trzy próby 15/30/60 s, wyczerpanie prób, końcowy `TECHNICAL_ERROR`) pozostają `IN_PROGRESS`.
...
- w sesji PR `feat/lab-rate-limit` nie było dostępu do MySQL ani Dockera, więc `npm run test:integration` i `npm run test:migration:lab-rate-limit` nie zostały uruchomione lokalnie — ich wynik pochodzi z CI i wymaga potwierdzenia w logach przebiegu, zanim scenariusz `RATE_LIMIT` będzie mógł zostać podniesiony do `VERIFIED`.
