 ...
 * test odtwarza dokładnie tę ścieżkę: wybór priorytetu Pilne w formularzu,
 * przechwycenie realnego żądania POST wysłanego przez przeglądarkę i
 * potwierdzenie, że mimo poprawnego działania backendu (zapisuje dokładnie
 * to, co dostał) szczegóły zlecenia pokazują błędny priorytet Rutynowe.
 */
test.describe("Journey 9 — ORDER_PRIORITY_MAPPING: CITO wysyłane jako ROUTINE", () => {
  test.skip(!WORKSHOP_E2E_CONFIRMED, "wymaga WORKSHOP_E2E_CONFIRM=RUN");

  test("ORDER_PRIORITY_MAPPING aktywny → wybór Pilne → request zawiera ROUTINE → szczegóły pokazują Rutynowe", async ({
    page,
    request
  }) => {
    ...
    try {
      ...
      // Network: uczestnik wybrał "Pilne" w UI, ale przeglądarka wysłała
      // ROUTINE — to jest dokładnie to, co inspekcja DevTools ma pokazać.
